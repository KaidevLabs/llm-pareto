...
// ---- 3D showcase (plan 023 D7): the echarts-gl seam ---------------------
// The vendored WebGL extension (see the provenance comment in index.html)
// is injected on demand only — 2D-only visits never download it. Same
// promise-cached pattern as fetchEndpoints (D5); the promise resolves true
// when the UMD global registered (the factory wires grid3D/scatter3D/
// lines3D onto the global echarts), false on any failure — a failed load
// must leave the 2D views untouched (the 3D pill reports it in step 5).
let glPromise = null;
